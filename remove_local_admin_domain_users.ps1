...
foreach($i in (Get-Content C:\Users\admin\Desktop\users.txt)){
 ...
 foreach($admin in $admins){
  ...
  if(($admin.users[$i] -ne "Contoso\Domain Admins") -and ($admin.users[$i] -notlike "Contoso\WkstPerm*") -and ($admin.users[$i] -notlike "Administrator") -and ($admin.users[$i] -ne $null)){
   ...
   # adds logged in use to remote desktop users group
    ...
    Invoke-Command -ComputerName $($admin.computername) -ScriptBlock $scriptblock -ErrorVariable remotedesktop
    }
     Catch{
   ...
   Catch{
    ...
    Invoke-Command -ComputerName $($admin.computername) { (gwmi win32_operatingsystem).Win32Shutdown(4) |Out-Null} -ErrorVariable logoff
